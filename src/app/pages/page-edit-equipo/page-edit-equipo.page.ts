...
@Component({
  selector: 'app-page-edit-equipo',
  templateUrl: './page-edit-equipo.page.html',
  styleUrls: ['./page-edit-equipo.page.scss'],
})
export class PageEditEquipoPage implements OnInit {
  ...
  async deleteTeam() {
    ...
    this.firestoreService.deletepartido(path, this.equipo.uid).then(res => {
      this.presentLoading('Eliminando', 1500);
      setTimeout(() => {
        this.router.navigate(['/tab-info-campeonato/page-equipos-campeonato']);
      }, 1000);
    }).catch(error => {
      console.log(error);
    });
